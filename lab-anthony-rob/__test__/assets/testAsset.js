...
let bmpHeader = [
  ...
  218, 58, 0, 0, // DA 3A 00 00 ->  File Size: 15066 bytes // BMP Header + DIB Header + (widthInPixels * heightInPixels) + (numColorsInPallette * 4) // Don't know where this is used?
...
let dibHeader = [
  ...
  3, 0, 0, 0, // 03 00 00 00 -> number of colors in the color pallette: 3 // Don't know where this is used?
...
// here we define the pixel array. each pixel is stored as a reference back to one of the colors in the color pallette, as indicated by their index. You should have the same number of pixels here as your width * height. Numbers go in row by bow, bottom left to bottom right, then up.
let pixelArray = [];
...
let bmpArray = bmpHeader.concat(dibHeader).concat(colorPallette).concat(pixelArray);
